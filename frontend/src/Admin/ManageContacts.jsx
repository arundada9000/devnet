...
export default function ManageContacts() {
  ...
  const { data: records = [], isLoading: loading } = useQuery({ queryKey: ["contacts"], queryFn: () => API.get("/contacts").then(res => res.data) });

  const deleteMutation = useMutation({
    mutationFn: (id) => API.delete(`/contacts/${id}`),
    onSuccess: () => { toast.success(t("admin.manageContacts.deleted")); queryClient.invalidateQueries({ queryKey: ["contacts"] }); },
    onError: () => toast.error(t("admin.manageContacts.deleteFailed")),
  });

  const filtered = useMemo(() => {
    let data = [...records];
    if (debouncedSearch) { const q = debouncedSearch.toLowerCase(); data = data.filter((r) => r.localGovName.toLowerCase().includes(q) || r.department.toLowerCase().includes(q) || r.contacts.some((c) => c.name.toLowerCase().includes(q))); }
  ...
  const handleDelete = (id) => { if (!window.confirm(t("admin.manageContacts.deleteConfirm"))) return; deleteMutation.mutate(id); };
    ...
    <>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => exportToCSV(filtered, "contacts")} disabled={filtered.length === 0}
          ...
          <button onClick={() => { setEditingRecord(null); setModalOpen(true); }}
            className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 hover:-translate-y-0.5 hover:shadow-md text-white px-4 py-2 rounded-lg shadow-sm font-medium text-sm transition-all active:scale-95">
            <Plus size={16} /> {t("admin.manageContacts.addNew")}
          </button>
        </div>
      </div>

      <AdminFilterBar search={search} onSearchChange={setSearch} searchPlaceholder={t("admin.manageContacts.searchPlaceholder")}
        ...
        onClearAll={() => { setSearch(""); setDeptFilter(""); setLocationFilter(""); }} />
            ...
            {loading ? <TableSkeleton rows={PAGE_SIZE} cols={4} /> : paginated.length === 0 ? (
              ...
              <motion.tr key={record._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.03 * i }} className="hover:bg-indigo-50/30 transition">
                ...
                <td className="px-6 py-4 text-sm"><div className="flex gap-2">
                  ...
                  <button onClick={() => { setEditingRecord(record); setModalOpen(true); }} className="flex items-center gap-1 px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 hover:-translate-y-0.5 hover:shadow-sm transition-all text-xs font-medium active:scale-95"><Edit3 size={13} /> {t("admin.manageContacts.edit")}</button>
                  <button onClick={() => handleDelete(record._id)} disabled={deleteMutation.isPending} className="flex items-center gap-1 px-3 py-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 hover:-translate-y-0.5 hover:shadow-sm transition-all text-xs font-medium active:scale-95 disabled:opacity-50"><Trash2 size={13} /> {t("admin.manageContacts.delete")}</button>
                </div></td>
              </motion.tr>
            ))}
          </tbody>
      ...
      <DetailModal open={viewOpen} onClose={() => { setViewOpen(false); setViewItem(null); }} title={t("admin.manageContacts.contactDetails")} icon={<Contact size={20} />}
        fields={[
      ...
      <AnimatePresence>
        {modalOpen && <ContactEditModal record={editingRecord} adminLocation={adminLocation} availableLocations={availableLocations}
          onClose={() => setModalOpen(false)} onSuccess={() => queryClient.invalidateQueries({ queryKey: ["contacts"] })} />}
      </AnimatePresence>
    </>
  );
}