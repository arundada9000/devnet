...
export default function AdminDashboard() {
  ...
  const typeCounts = useMemo(() => {
    const c = {};
    filteredReports.forEach((r) => { const type = r.type ? r.type.charAt(0).toUpperCase() + r.type.slice(1) : "Unknown"; c[type] = (c[type] || 0) + 1; });
    return c;
  }, [filteredReports]);

  const roleCounts = useMemo(() => users.reduce((a, u) => {
    const role = u.role ? u.role.charAt(0).toUpperCase() + u.role.slice(1) : "User";
    a[role] = (a[role] || 0) + 1; return a;
  }, {}), [users]);

  const alertsByType = useMemo(() => {
    const c = {};
    filteredAlerts.forEach((a) => { const type = a.type ? a.type.charAt(0).toUpperCase() + a.type.slice(1) : "Unknown"; c[type] = (c[type] || 0) + 1; });
    return c;
  }, [filteredAlerts]);
  ...
  const groupedCards = [
    { id: "reports", title: t("admin.dashboard.reports"), icon: <FileText className="text-indigo-600" size={24} />, total: filteredReports.length, bg: "bg-indigo-50", border: "border-indigo-100", link: "/admin/manage-reports",
      subcards: [{ label: t("admin.dashboard.active"), value: statusCounts.pending + statusCounts.working, color: "text-amber-600" }, { label: t("admin.dashboard.resolved"), value: statusCounts.verified + statusCounts.solved, color: "text-emerald-600" }] },
    { id: "users", title: t("admin.dashboard.users"), icon: <Users className="text-blue-600" size={24} />, total: users.length, bg: "bg-blue-50", border: "border-blue-100", link: "/admin/manage-users",
      subcards: [{ label: t("admin.dashboard.admins"), value: roleCounts.Admin || 0, color: "text-purple-600" }, { label: t("admin.dashboard.users"), value: roleCounts.User || 0, color: "text-blue-600" }] },
    { id: "alerts", title: t("admin.dashboard.alerts"), icon: <Bell className="text-amber-600" size={24} />, total: filteredAlerts.length, bg: "bg-amber-50", border: "border-amber-100", link: "/admin/send-alerts", subcards: alertSubcards },
    { id: "safe-zones", title: t("admin.dashboard.safeZones"), icon: <Shield className="text-emerald-600" size={24} />, total: filteredSafeZones.length, bg: "bg-emerald-50", border: "border-emerald-100", link: "/admin/manage-safe-zones",
      subcards: [{ label: t("admin.dashboard.active"), value: safeZoneStats.active, color: "text-emerald-600" }, { label: t("admin.dashboard.inactive"), value: safeZoneStats.inactive, color: "text-gray-500" }] },
    { id: "contacts", title: t("admin.dashboard.contacts"), icon: <Contact className="text-rose-600" size={24} />, total: filteredContacts.length, bg: "bg-rose-50", border: "border-rose-100", link: "/admin/manage-contacts",
      subcards: [{ label: t("admin.dashboard.depts"), value: contactStats.depts, color: "text-rose-600" }, { label: t("admin.dashboard.phones"), value: contactStats.phones, color: "text-orange-600" }] },
  ];

  const SkeletonCard = () => (
    <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-gray-200 animate-pulse" /><div className="w-24 h-6 bg-gray-200 animate-pulse rounded" /></div>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <h1 className="text-3xl font-bold text-gray-800 tracking-tight flex items-center gap-3 hover:text-indigo-600 transition-colors cursor-pointer">
            <LayoutDashboard className="text-indigo-500" size={32} />{t("admin.dashboard.title")}
          </h1>
        ...
        <div className="flex items-center gap-2 bg-white p-1.5 rounded-xl border border-gray-200 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 transition-shadow">
          ...
          <select value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)}
            className="px-2 py-1.5 bg-transparent text-sm font-medium text-gray-700 focus:outline-none cursor-pointer" aria-label="Filter by location">
            ...
            <motion.div key={card.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl ${card.bg} flex items-center justify-center group-hover:scale-110 transition-transform`}>{card.icon}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
          <h2 className="text-base font-bold text-gray-800 mb-4 flex items-center gap-2">
            <FileText size={18} className="text-indigo-500" />{t("admin.dashboard.reportsByType")}
          </h2>
          <div className="flex-1 relative min-h-[280px]">
            {loading ? <SkeletonChart /> : Object.keys(typeCounts).length > 0 ? (
              <Bar data={{ labels: Object.keys(typeCounts), datasets: [{ label: t("admin.dashboard.reports"), data: Object.values(typeCounts), backgroundColor: "rgba(99, 102, 241, 0.8)", borderRadius: 6, hoverBackgroundColor: "rgba(79, 70, 229, 1)" }] }}
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
          <h2 className="text-base font-bold text-gray-800 mb-4 flex items-center gap-2">
            <PieChart className="text-blue-500" size={18} />{t("admin.dashboard.reportStatus")}
          </h2>
          <div className="flex-1 relative min-h-[280px] flex items-center justify-center">
            {loading ? <SkeletonChart /> : filteredReports.length > 0 ? (
              <Doughnut data={{ labels: [t("charts.pending"), t("charts.verified"), t("charts.working"), t("charts.solved")], datasets: [{ data: [statusCounts.pending, statusCounts.verified, statusCounts.working, statusCounts.solved], backgroundColor: ["#fbbf24", "#3b82f6", "#a855f7", "#10b981"], borderWidth: 0, hoverOffset: 4 }] }}
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
          <h2 className="text-base font-bold text-gray-800 mb-4 flex items-center gap-2">
            <Bell size={18} className="text-amber-500" />{t("admin.dashboard.alertsDistribution")}
          </h2>
          <div className="flex-1 relative min-h-[280px] flex items-center justify-center">
            {loading ? <SkeletonChart /> : filteredAlerts.length > 0 ? (
              <Pie data={{ labels: Object.keys(alertsByType), datasets: [{ data: Object.values(alertsByType), backgroundColor: ["#f87171", "#fb923c", "#fbbf24", "#a3e635", "#38bdf8", "#818cf8"], borderWidth: 1 }] }}
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
          <h2 className="text-base font-bold text-gray-800 mb-4 flex items-center gap-2">
            <Users size={18} className="text-purple-500" />{t("admin.dashboard.usersOverview")}
          </h2>
          <div className="flex-1 relative min-h-[280px] flex items-center justify-center">
            {loading ? <SkeletonChart /> : users.length > 0 ? (
              <Bar data={{ labels: Object.keys(roleCounts), datasets: [{ label: t("admin.dashboard.users"), data: Object.values(roleCounts), backgroundColor: ["#c084fc", "#60a5fa"], borderRadius: 8 }] }}
