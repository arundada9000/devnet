...
export default function Reports() {
  ...
  const { data: reports = [], isLoading: loading } = useQuery({ queryKey: ["reports"], queryFn: () => API.get("/reports").then((res) => res.data) });

  const filtered = useMemo(() => {
    let data = [...reports];
    if (showSpam) { data = data.filter((r) => r.aiAnalysis?.isFake === true); }
    else { data = data.filter((r) => r.aiAnalysis?.isFake !== true); }
    if (debouncedSearch) { const q = debouncedSearch.toLowerCase(); data = data.filter((r) => r.description?.toLowerCase().includes(q) || r._id?.toLowerCase().includes(q)); }
  ...
  const handleUpdate = () => { queryClient.invalidateQueries({ queryKey: ["reports"] }); setShowModal(false); };

  const handlePingVolunteers = async () => {
    if (!viewItem) return;
    setPinging(true);
    try { const { data } = await API.post(`/reports/${viewItem._id}/ping-volunteers`); toast.success(data.message || t("admin.manageReports.pingSuccess")); }
    catch (err) { toast.error(err.response?.data?.message || t("admin.manageReports.pingError")); }
  ...
  const handleSendReply = async () => {
    ...
    try { const { data } = await API.post("/webhook/sms/reply", { phone: viewItem.reportedByPhone, message: replyMessage }); toast.success(data.message || t("admin.manageReports.replySuccess")); setReplyMessage(""); setReplyOpen(false); }
    catch (err) { toast.error(err.response?.data?.message || t("admin.manageReports.replyError")); }
    finally { setSendingReply(false); }
  };

  const handleFindNearby = async () => {
    if (!viewItem?.location?.coordinates) return;
    setFetchingNearby(true); setShowNearby(true);
    try { const [lng, lat] = viewItem.location.coordinates; const { data } = await API.get(`/auth/volunteers/nearby?longitude=${lng}&latitude=${lat}&radius=10`); setNearbyVolunteers(data); }
    catch { toast.error(t("admin.manageReports.errorNearby")); }
    finally { setFetchingNearby(false); }
  };

  const counts = useMemo(() => {
    const c = { pending: 0, verified: 0, working: 0, solved: 0, spam: 0, totalReal: 0 };
    reports.forEach((r) => { if (r.aiAnalysis?.isFake) c.spam++; else { c.totalReal++; if (c[r.status] !== undefined) c[r.status]++; } });
    return c;
  }, [reports]);
    ...
    <>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        <h1 className="text-3xl font-bold text-gray-800 tracking-tight flex items-center gap-3 hover:text-indigo-600 transition-colors cursor-pointer">
          <FileText className="text-indigo-500" size={32} />{t("admin.manageReports.title")}
        </h1>
        <button onClick={() => exportToCSV(filtered, "reports")} disabled={filtered.length === 0}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        {[
          ...
          <motion.div key={label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
            ...
            className="rounded-xl p-4 shadow-sm text-center flex flex-col items-center cursor-pointer hover:shadow-md hover:-translate-y-1 active:scale-95 transition-all" style={{ backgroundColor: color.split(" ")[0], color: color.split(" ")[1] }}>
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setShowSpam(false)} className={`px-4 py-2 rounded-lg text-sm font-semibold transition ${!showSpam ? "bg-indigo-600 text-white shadow-md" : "bg-white text-gray-600 border border-gray-200 hover:bg-gray-50"}`}>{t("admin.manageReports.verifiedReports", "Verified Reports")}</button>
      ...
      <AdminFilterBar search={search} onSearchChange={setSearch} searchPlaceholder={t("admin.manageReports.searchPlaceholder")}
        ...
        onClearAll={() => { setSearch(""); setStatusFilter(""); setTypeFilter(""); setLocationFilter(""); }} />

      <div className="overflow-x-auto bg-white rounded-2xl shadow-lg ring-1 ring-gray-200">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50"><tr>{[t("admin.manageReports.tableType"), t("admin.manageReports.tableStatus"), t("admin.manageReports.tableDescription"), t("admin.manageReports.tableLocation"), t("admin.manageReports.tableActions")].map((h) => (<th key={h} className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{h}</th>))}</tr></thead>
          <tbody className="divide-y divide-gray-100">
            {loading ? <TableSkeleton rows={PAGE_SIZE} cols={5} /> : paginated.length === 0 ? (
              ...
              <motion.tr key={r._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="hover:bg-indigo-50/30 transition">
                <td className="px-6 py-4 text-sm font-medium text-gray-800 flex items-center gap-2"><img src={`/icons/map-icons-red/${r.type}.svg`} alt={r.type} className="w-5 h-5" /><span className="capitalize">{r.type}</span></td>
      ...
      <DetailModal open={viewOpen} onClose={() => { setViewOpen(false); setViewItem(null); setShowNearby(false); setReplyOpen(false); }}
        title={t("admin.manageReports.reportDetails")} icon={<FileText size={20} />}
        ...
        fields={[
          ...
          { label: t("admin.manageReports.statusLabel", "Status"), value: viewItem?.status, render: (v) => <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${statusColor(v)}`}>{v}</span> },
          { label: t("admin.manageReports.description", "Description"), value: viewItem?.description },
          ...(viewItem?.reportedByPhone ? [{
            label: t("admin.manageReports.reportedByPhone", "Reported via SMS"), value: viewItem.reportedByPhone,
            render: (v) => (
              <div className="flex flex-col gap-2 mt-1">
                <div className="flex items-center gap-3">
                  <a href={`tel:${v}`} className="text-blue-600 font-semibold hover:underline">{v}</a>
                  <button onClick={() => setReplyOpen(!replyOpen)} className="text-xs bg-indigo-50 text-indigo-700 px-2 py-1 rounded border border-indigo-200 hover:bg-indigo-100 transition font-bold">{t("admin.manageReports.replyViaSms", "Reply via SMS")}</button>
                </div>
                {replyOpen && (
                  <div className="flex flex-col gap-2 mt-2 bg-gray-50 p-3 rounded-lg border border-gray-200 shadow-inner">
                    <textarea value={replyMessage} onChange={(e) => setReplyMessage(e.target.value)} placeholder={t("admin.manageReports.typeSms", "Type SMS message...")} className="w-full text-sm p-2 rounded border border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500" rows={2} />
                    <button onClick={handleSendReply} disabled={sendingReply || !replyMessage.trim()} className="self-end px-4 py-1.5 bg-indigo-600 text-white text-xs font-bold rounded shadow-sm hover:bg-indigo-700 disabled:opacity-50 transition">
          ...
          ...(viewItem?.aiAnalysis ? [
            { label: t("admin.manageReports.aiSeverity", "AI Severity Score"), value: viewItem.aiAnalysis.severityScore, render: (v) => <span className={`font-bold ${v > 7 ? 'text-red-600' : v > 4 ? 'text-yellow-600' : 'text-green-600'}`}>{v}/10</span> },
          ...
          { label: t("admin.manageReports.coordinates", "Coordinates"), value: viewItem?.location?.coordinates ? `[${viewItem.location.coordinates[0]?.toFixed(5)}, ${viewItem.location.coordinates[1]?.toFixed(5)}]` : "\u2014" },
          ...(showNearby ? [{
            label: t("admin.manageReports.nearbyVolunteers", "Nearby Volunteers (10km)"), value: null,
            render: () => (
              <div className="mt-2 flex flex-col gap-2">
                {fetchingNearby ? <span className="text-sm text-gray-500">{t("admin.manageReports.loadingNearby", "Loading...")}</span>
                : nearbyVolunteers.length === 0 ? <span className="text-sm text-gray-500">{t("admin.manageReports.noNearbyFound", "No volunteers found within 10km.")}</span>
                    ...
                    {nearbyVolunteers.map(v => (
                      <div key={v._id} className="flex justify-between items-center text-sm p-2 border border-gray-100 bg-white rounded shadow-sm">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-gray-500 flex items-center gap-1">{v.phoneNumber && <a href={`tel:${v.phoneNumber}`} className="text-blue-600 hover:underline">{v.phoneNumber}</a>}</span>
                        </div>
                        <div className="text-xs text-indigo-600 font-bold bg-indigo-50 px-2 py-1 rounded">{v.skills?.length > 0 ? v.skills.join(", ") : "General"}</div>
                      </div>
                    ))}
                  </div>}
              </div>
        ...
        actions={
          <>
            <button onClick={handleFindNearby} disabled={fetchingNearby || !viewItem?.location?.coordinates}
            ...
            <button onClick={handlePingVolunteers} disabled={pinging}
