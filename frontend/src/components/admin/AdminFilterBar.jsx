 ...
 * Renders a search input, a location dropdown (seeded from useAdminLocationStore),
 * and any additional custom filter dropdowns passed as children.
 *
 * @param {string}   search          - Current search value
 * @param {function} onSearchChange  - Callback for search input changes
 * @param {string}   searchPlaceholder
 * @param {string}   locationFilter  - Currently selected location
 * @param {function} onLocationChange
 * @param {Array}    filters         - Array of { label, value, onChange, options: [{value, label}] }
 * @param {function} onClearAll      - Callback to reset all filters
 */
  ...
  return (
    <div className="bg-white/70 backdrop-blur-lg border border-gray-200 shadow-sm rounded-xl p-4 mb-6 flex flex-wrap gap-4 items-end">
      ...
      {hasActiveFilters && onClearAll && (
        <button
          onClick={onClearAll}
          className="flex items-center gap-1.5 px-3 py-2 text-sm text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg border border-gray-200 transition self-end"
        >
<X size={14} />
          {t("adminFilter.clear")}
          </button>
