...
export default function AlertModal({ open, onClose, onSubmit, initialData, submitting }) {
  ...
  useEffect(() => {
    if (initialData) setForm({ title: initialData.title || "", description: initialData.description || "", type: initialData.type || "", location: initialData.location || "" });
    else setForm({ title: "", description: "", type: "", location: "" });
  }, [initialData, open]);
    ...
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }}
          ...
          className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
          <button onClick={onClose} className="absolute top-4 right-4 p-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-500 transition">
            <X size={18} />
          </button>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-bold text-gray-800">{initialData ? t("alertModal.editAlert") : t("alertModal.broadcastAlert")}</h2>
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              <div className="absolute top-3 left-3 text-gray-400"><Tag size={18} /></div>
              <input name="title" value={form.title} onChange={handleChange} placeholder={t("alertModal.alertTitle")} required
            ...
            <div className="relative">
              <div className="absolute top-3 left-3 text-gray-400"><MapPin size={18} /></div>
              <input name="location" value={form.location} onChange={handleChange} placeholder={t("alertModal.affectedLocation")} required
                className="w-full border border-gray-200 pl-10 pr-4 py-2.5 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition" />
            </div>
            <div className="relative">
              <textarea name="description" value={form.description} onChange={handleChange} placeholder={t("alertModal.detailedDescription")}
                rows={4} required
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={onClose}
                className="px-5 py-2.5 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold text-sm transition">{t("alertModal.cancel")}</button>
