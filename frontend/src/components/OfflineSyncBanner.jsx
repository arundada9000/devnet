...
const OfflineSyncBanner = () => {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (result.failed > 0) toast.error(t("offlineSync.failed", { count: result.failed }));
      await checkPending();
    } catch {
  ...
  useEffect(() => {
    const handleOnline = () => { if (pendingCount > 0) handleSync(); };
    window.addEventListener("online", handleOnline);
    return () => window.removeEventListener("online", handleOnline);
  }, [pendingCount]);

  useEffect(() => {
    const handler = (event) => { if (event.data?.type === "SYNC_REPORTS" && pendingCount > 0) handleSync(); };
      ...
      <motion.div initial={{ opacity: 0, y: -10, height: 0 }} animate={{ opacity: 1, y: 0, height: "auto" }} exit={{ opacity: 0, y: -10, height: 0 }} className="mb-4">
        {justSynced ? (
          <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-2xl p-4">
            ...
            <p className="text-sm font-medium text-green-800">{t("offlineSync.syncedAll")}</p>
          </div>
        ) : (
          <div className="flex items-center justify-between gap-3 bg-amber-50 border border-amber-200 rounded-2xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-amber-900">{t("offlineSync.pendingSync", { count: pendingCount })}</p>
                <p className="text-xs text-amber-700 mt-0.5">{navigator.onLine ? t("offlineSync.onlineTap") : t("offlineSync.autoSync")}</p>
              </div>
            </div>
            {navigator.onLine && (
