...
import {
  Home, Menu, X, LayoutDashboard, FileText, Bell, Users, Phone,
  LogOut, ChevronLeft, Shield, HeartHandshake,
} from "lucide-react";
...
export default function AdminSidebar() {
  ...
  return (
    <>
      ...
      {mobileOpen && <div className="md:hidden fixed inset-0 bg-black/40 z-[69]" onClick={() => setMobileOpen(false)} />}
      <aside className={`top-0 h-screen z-[70] bg-gray-900 text-white flex flex-col shrink-0 transition-all duration-300 ease-in-out
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-800">
          {!collapsed && <span className="text-lg font-bold tracking-tight">{t("sidebar.admin")}</span>}
        ...
        <div className="p-3 border-t border-gray-800">
          <button onClick={() => navigate("/logout")}
            ...
            <LogOut size={18} className="shrink-0" />{!collapsed && <span>{t("sidebar.logout")}</span>}
          </button>
          {!collapsed && <p className="text-[11px] text-gray-600 mt-3 px-3">{t("sidebar.footer")}</p>}
        </div>
      </aside>
    </>
  );
