...
import {
  Camera, MapPin, RefreshCcw, CheckCircle2, AlertTriangle,
  Loader2, X, ChevronLeft, Flame, Waves, Mountain, Car,
  Trash2, AlertCircle, WifiOff
} from "lucide-react";
...
export default function ReportForm() {
  ...
  const detectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { toast.error(t("report.locationError", "Failed to secure location.")); setLocating(false); },
  ...
  const handleVideoLoaded = () => setCameraReady(true);
  ...
  const clearPhoto = () => { setPreview(null); setPhotoFile(null); setMediaSource(""); };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!location) { detectLocation(); return toast.error(t("report.locationFetching")); }
    if (!photoFile) return toast.error(t("report.livePhotoRequired"));
    if (description.length < 10) return toast.error(t("report.descriptionLength"));
    if (!type || !tags.map((t) => t.id).includes(type)) return toast.error(t("report.invalidType"));
    ...
    try {
      ...
      console.error(err);
      try {
        const offlineData = await prepareOfflineReport({ type, description, location: locationString, photoFile });
        ...
        <AnimatePresence mode="wait">
          ...
          {status === "queued" && (
            <motion.div key="queued" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-10 rounded-[2rem] shadow-2xl max-w-sm w-full mx-4 text-center">
              ...
              <p className="text-sm text-gray-600 mb-6">{t("report.queuedMessage", "Your report will sync when back online.")}</p>
          ...
          {status === "failed" && (
            <motion.div key="failed" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="bg-white p-10 rounded-[2rem] shadow-2xl max-w-sm w-full mx-4 text-center">
              <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6">
                <AlertTriangle size={40} className="text-red-500" />
              </div>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">{t("report.errorTitle", "Submission Failed")}</h2>
              <p className="text-red-600 mb-8 font-medium">{errorMessage}</p>
              <div className="flex gap-3">
                <button onClick={() => setStatus("idle")} className="flex-1 bg-gray-100 text-gray-700 font-bold py-3 rounded-xl hover:bg-gray-200 transition">{t("report.cancel", "Cancel")}</button>
                <button onClick={handleSubmit} className="flex-1 bg-red-600 text-white font-bold py-3 rounded-xl hover:bg-red-700 transition">{t("report.retry", "Retry")}</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <main className="max-w-xl mx-auto p-4 space-y-6 mt-4">
        <form onSubmit={handleSubmit} className="space-y-6">
          <motion.section initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" onClick={detectLocation} disabled={locating}
                className="text-xs font-semibold text-blue-600 flex items-center gap-1 bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition disabled:opacity-50">
              ...
              {locating ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-mono mt-0.5">{location.lat.toFixed(5)}, {location.lng.toFixed(5)}</p>
                  </div>
                </div>
              ) : (
                <div className="flex items-center gap-3 text-red-700">
                  <AlertTriangle size={18} /> <span className="text-sm font-medium">{t("report.locationError", "Failed to retrieve location.")}</span>
                </div>
              )}
            </div>
          </motion.section>
              ...
              <AnimatePresence mode="wait">
                {showCamera ? (
                  <motion.div key="camera" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative rounded-xl overflow-hidden bg-black">
                    <video ref={videoRef} autoPlay playsInline onCanPlay={handleVideoLoaded} className="w-full aspect-[4/3] object-cover" />
                    {!cameraReady && <div className="absolute inset-0 flex items-center justify-center"><Loader2 className="animate-spin text-white w-8 h-8" /></div>}
                  ...
                  <motion.div key="preview" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="relative rounded-xl overflow-hidden shadow-md">
                    ...
                    <button type="button" onClick={clearPhoto} className="absolute top-3 right-3 bg-red-500 hover:bg-red-600 text-white p-2 rounded-full shadow-lg transition"><X size={18} /></button>
                    <button type="button" onClick={openCamera} className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-md text-gray-800 px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-white transition shadow">
                      {t("report.retake")}
                    </button>
                  </motion.div>
                ) : (
          ...
          <motion.section initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-5">
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {tags.map((tItem) => (
                  <button key={tItem.id} type="button" onClick={() => setType(tItem.id)}
          ...
          <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="pt-2">
            <button type="submit" disabled={status === "submitting" || locating}
